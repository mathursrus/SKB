...
5. Diner submits → existing happy-path response. The form is replaced by **two stacked cards**: (a) a compact position card showing place-in-line / code / promised time / hint, and (b) the **full public waitlist** (R3 from issue #37 — every party in line, names redacted to first name + last initial, the diner's own row highlighted). The status strip stays at the top, and the public-list card MUST remain visible without requiring the diner to scroll past more than the position card.
6. If the diner returns later (deep link `?code=X`), they see the position card + public list directly — same data shape as today; the boot-flicker handling in `queue.js` (`.queue-boot` → `.queue-ready`) is unchanged.
7. The post-join chat card (`#chat-card` — only visible after the host sends the first inbound message) and the "called" callout / "I'm on my way" ack button (`#called-callout`, `#ack-btn`) MUST continue to render in their existing positions and behave identically; this redesign only restyles, it does not remove any post-join surface.
...
2. **At < 720 px (phone):**
   ...
   - Each tab's `<table>` is **replaced by a stacked card list with full feature parity** — no actions, badges, or metric columns are removed; the layout flow just changes from row-by-row table cells to a vertical card stack. Specifically:
     - **Waiting card** preserves all 8 row actions from `host.js:144-156` — Sentiment select · Seat · Notify (or "Re-notify" when already called) · Chat (with unread-count dot) · Call (instant `tel:` link) · Custom SMS (✉, opens compose modal) · Custom Call (☎, confirm-then-dial — distinct from instant Call because shared devices need a confirmation) · No-show. Plus all status badges (`called`, `on-way`, sentiment dot, unread chat). The card lays them out as: a primary action row (Seat + Notify) for the highest-frequency host moves, a secondary row of 4 icon-with-label buttons (Call / Chat / SMS / Conf), and a tertiary row with the sentiment select + No-show button. All buttons ≥ 44 × 44 px.
     - **Seated card** preserves all 7 metric columns from the desktop `<table>` — Waited / To Order / To Serve / To Checkout (rendered as a 4-up "transition pills" row), state badge, In State timer, Total at Table — plus the existing state-advance ladder (`NEXT_ACTION` ladder in `host.js:173-178`: seated → ordered → served → checkout → departed) as a primary "next-state" button (label depends on current state), the "Departed" shortcut button, the sentiment select, and the tap-to-expand timeline (`expandedTimelineId` flow in `host.js:233-241`).
     - **Complete card** preserves all completion metrics (waited / to-order / to-serve / dining / paying / total) in the same transition-pill grid, with state badge.
...
3. **At ≥ 720 px (tablet/desktop):** the existing table layouts are preserved unchanged — same DOM, same JS handlers, only the mobile branch in CSS swaps the row rendering. The breakpoint is the single switch between card-stack and table — no separate code path.
...
| **R11** | Once the diner has joined the waitlist, the public-list view (R3, issue #37 — every party in line, names redacted to first name + last initial) SHALL remain visible without requiring scroll past the position card. The position card SHALL be sized so the first row of the public-list card is in-viewport on a 375 × 667 phone. | `public/queue.html`, `public/styles.css` |
| **R12** | The mobile redesign of `/host` MUST preserve full feature parity with the desktop table layout. Specifically: (a) every row action present in `host.js:144-156` (Waiting) and `host.js:209-217` (Seated) SHALL render on each mobile card; (b) every status badge (called, on-way, sentiment, unread) SHALL render on the mobile card; (c) every transition-duration metric (Waited / To Order / To Serve / To Checkout / In State / Total) SHALL render on each Seated mobile card; (d) the state-advance ladder (`NEXT_ACTION`) SHALL be preserved as the card's primary action with the same label semantics; (e) the tap-to-expand timeline behavior SHALL be preserved. **No host-side feature is removed in the name of mobile cleanup.** | `public/host.html`, `public/host.js`, `public/styles.css` |
...
- **AC-R11.** *Given* a diner who has just joined the waitlist on a 375 × 667 viewport, *when* the post-join page renders, *then* both the position card AND at least the first row of the `Who's in line` public-list card SHALL be visible without scrolling.
- **AC-R12.** *Given* an automated audit at viewport width 375 px, *when* the Waiting tab is rendered with at least one party, *then* the rendered card SHALL contain DOM nodes equivalent to each of: `[data-action="sentiment"]`, `[data-action="seat"]`, `[data-action="notify"]`, `[data-action="chat"]`, `[data-action="call"]`, `[data-action="custom-sms"]`, `[data-action="custom-call"]`, and a no-show control (matching `host.js:144-156`). Same audit on the Seated tab SHALL find the sentiment select, the advance-to-next-state button, the Departed button, and DOM nodes carrying every transit-duration value the desktop table renders.
...
2a. **Action-set parity audit (R12).** A dedicated test SHALL render the host page at 375 px with seeded waiting + dining + complete parties, then assert every selector listed in AC-R12 is present in the DOM. The same test re-runs at 1280 px and asserts the equivalent table-cell selectors are present — proving both layouts surface the full action set.
2b. **Post-join public-list visibility (R11).** A test SHALL navigate the diner page through join, then assert at viewport 375 × 667 that both `#conf-card` (or its replacement) and the first `.public-list-rows > *` are within `window.innerHeight` (no scroll required).
...
| R11 | UX (A) step 5–7, Acceptance criteria AC-R11 |
| R12 | UX (B) step 2 (Waiting / Seated / Complete card bullets), Acceptance criteria AC-R12 |
